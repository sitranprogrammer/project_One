        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <div class="card card-primary">
              ...
              <!-- /.card-header -->
              <!-- form start -->
              <form role="form" action="{{route('san-pham.update',$edit->id)}}" method="post" enctype="multipart/form-data">
              @method('PATCH')
              @csrf
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <select name="id_chill" id="id_chill" class="form-control @foreach($errors->get('id_chill') as $err){{$err}} @endforeach">
                      @foreach($catalog_2 as $cata_2)
                        @if($cata_2->id == $edit->id_chill)
                          <option value="{{$cata_2->id}}" selected>{{$cata_2->name}}</option>
                          @else
                          <option value="{{$cata_2->id}}">{{$cata_2->name}}</option>
                        @endif
                      @endforeach
                    </select>
                  </div>
